...
def fetch_keys(yaw_c,altitude_c,running):
    ...
    if keys[QUIT]:
        running = False
    return u_c,v_c,yaw_c,altitude_c,running

while running:

    pygame.event.pump() #this clears events to make sure fresh ones come in
    u_c, v_c, yaw_c, altitude_c, running= fetch_keys(yaw_c,altitude_c,running)
